...
export class VectorTileFeature {
     ...
     * @param encrypt
     */
    constructor(pbf, end, extent, keys, values, encrypt) {
        // Public
    ...
    loadGeometry() {
        ...
        while (pbf.pos < end) {
            ...
            if (isPoint(cmd, this.encrypt)) {
                x += pbf.readSVarint();
                y += pbf.readSVarint();

                if (isMoveTo(cmd, this.encrypt)) { // moveTo
...
function readFeature(tag, feature, pbf) {
    tag = toMapboxFeatureTag(tag, feature.encrypt);
    if (tag === 1) feature.id = pbf.readVarint();
    else if (tag === 2) readTag(pbf, feature);
    else if (tag === 3) feature.type = toMapboxFeatureType(pbf.readVarint(), feature.encrypt);
...
function readValueMessage(pbf, encrypt) {
    ...
    while (pbf.pos < end) {
        const tag = toMapboxValueMessageTag(pbf.readVarint() >> 3, encrypt);
...
function readTile(options, tag, layers, pbf) {
    if (tag === 3) {
        const layer = new VectorTileLayer(pbf, pbf.readVarint() + pbf.pos, options);
